...
def forward_backward_prop(X, labels, params, dimensions):
    ...
    gradW2 = hidden[:,:,np.newaxis] * gradb2[:,np.newaxis,:] # R: M*H*1 * M*1*Dy = M*H*Dy

    gradb1 = np.sum(gradb2[:,np.newaxis,:] * W2[np.newaxis,:,:],axis=2) * sigmoid_grad(hidden)  # R: = sum(M*1*Dy * M*Dy*H) * M*H= M*H
    gradW1 = X[:,:,np.newaxis] * gradb1[:,np.newaxis,:]  #R: M*H*1 * M*1*Dx = M*Dx*H
