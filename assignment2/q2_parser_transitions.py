...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        minibatchs = unfinished_parses[:batch_size]
        while minibatchs:
            transitions = model.predict(minibatchs)
            for i in range(len(minibatchs)):
                minibatchs[i].parse_step(transitions[i])
            minibatchs = [minibatch for minibatch in minibatchs 
                          if len(minibatch.stack)>1 or len(minibatch.buffer)>0]
